...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!typewriterDeleting && typewriterText.length < word.length) {
      ...
      timeout = setTimeout(() => setTypewriterDeleting(true), 1500);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10">
            {filteredProducts.map((product) => (
              <motion.div
                ...
                className="glass-card rounded-[2.5rem] overflow-hidden group relative transition-all duration-500 cursor-pointer"
              >
